"""FastAPI application for open-model-arena."""
...
app = FastAPI(title="open-model-arena", version="0.1.0", description="open-model-arena API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "open-model-arena", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "open-model-arena", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "open-model-arena", "version": "0.1.0", "ready": True,
